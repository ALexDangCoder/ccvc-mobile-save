...
class _TextSelectionControlsToolbarState
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.boxTextFeild == null) {
      anchorX = widget.globalEditableRegion.left + widget.selectionMidpoint.dx;
    } else {
      final offsetBoxX = widget.boxTextFeild!.localToGlobal(Offset.zero).dx;
      final boxWidth = widget.boxTextFeild!.size.width;
      final firstPointX = math
          .max(widget.endpoints.first.point.dx + offsetBoxX, offsetBoxX)
          .clamp(offsetBoxX, offsetBoxX + boxWidth);
      final lastPointX = math
          .min(
            widget.endpoints.last.point.dx,
            boxWidth,
          )
          .clamp(0, boxWidth);
      final userOffset = firstPointX + lastPointX / 2;
      anchorX = userOffset;
    }
