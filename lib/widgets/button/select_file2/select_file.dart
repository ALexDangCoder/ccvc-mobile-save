...
class SelectFileBtn2 extends StatefulWidget {
  ...
  final Function(List<BytesFileModel> fileSelected) onChange;
  final List<FileModel>? initFileFromApi;
  final List<BytesFileModel>? initFileSystem;
  ...
  @override
  State<SelectFileBtn2> createState() => _SelectFileBtn2State();
}

class _SelectFileBtn2State extends State<SelectFileBtn2> {
  ...
  Future<void> handleButtonFileClicked() async {
    ...
    final List<BytesFileModel> listFileBase64 = [];
    for (final file in newFiles) {
      listFileBase64.add(
        BytesFileModel(
            fileLength: file.lengthSync(),
            base64File: file.readAsBytesSync(),
            path: file.path.convertNameFile(),
        ),
      );
    }
    cubit.selectedFiles.addAll(listFileBase64);
                ...
                children: cubit.selectedFiles
                    .map(
                      (file) => itemListFile(
                        ...
                        lengthFile: file.fileLength.getFileSize(2),
                      ),
                    )
                    .toList(),
              );
