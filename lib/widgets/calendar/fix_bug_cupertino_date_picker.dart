  ...
  double _getEstimatedColumnWidth(_PickerColumnType columnType) {
    if (estimatedColumnWidths[columnType.index] == null) {
      estimatedColumnWidths[columnType.index] =
          FixBugCupertinoDatePicker._getColumnWidth(
    ...
    return NotificationListener<ScrollNotification>(
      ...
      child: CupertinoPicker(
        ...
        children: List<Widget>.generate(31, (int index) {
          ...
          final currentDay = DateTime(selectedYear, selectedMonth, day);
          bool afterMaxDate = false;
          bool beforeMinDate = false;
          if (widget.maximumDate != null) {
            afterMaxDate = currentDay.isAfter(widget.maximumDate!);
          }
          if (widget.minimumDate != null) {
            beforeMinDate = currentDay.isBefore(widget.minimumDate!);
          }
          final isDisable =
              day > daysInCurrentMonth || afterMaxDate || beforeMinDate;
          return itemPositioningBuilder(
            context,
            Text(
              localizations.datePickerDayOfMonth(day),
              style: _themeTextStyle(context, isValid: !isDisable),
            ),
          );
        }),
      ),
