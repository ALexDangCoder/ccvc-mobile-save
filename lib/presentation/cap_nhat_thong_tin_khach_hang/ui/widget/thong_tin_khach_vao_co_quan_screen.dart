...
class ThongTinKhachVaoCoQuanScreen extends StatefulWidget {
  ...
  ThongTinKhachVaoCoQuanScreen({
    Key? key,
    required this.checkIdCardModel,
...
class _ThongTinKhachVaoCoQuanScreenState
  ...
  void initState() {
    ...
    hoTenEditingController.text =
        widget.checkIdCardModel.frontContent?.content?.name ?? '';
    soCmndEditingController.text =
        widget.checkIdCardModel.frontContent?.content?.id ?? '';
    birth = widget.checkIdCardModel.frontContent?.content?.dob?.contains('-');
  }
              ...
              children: [
                ...
                InputInfoUserWidget(
                  ...
                  child: SelectDate(
                    ...
                    value: (widget.checkIdCardModel.frontContent?.content?.dob)
                            ?.convertStringToDate(
                              formatPattern: birth == false
                ...
                InputInfoUserWidget(
                  ...
                  child: CoolDropDown(
                    key: UniqueKey(),
                    initData:
                        widget.checkIdCardModel.frontContent?.content?.gender ??
                            'Nam',
                    placeHoder: S.current.gioi_tinh,
                    onChange: (value) {
                      value == 0 ? cubit.gioitinh = '1' : cubit.gioitinh = '2';
                    },
                ...
                InputInfoUserWidget(
                  ...
                  child: TextFieldValidator(
                    ...
                    validator: (value) {
                      return (value ?? '')
                          .checkTruongNull(S.current.nhap_ly_do);
                    },
                  ),
                ),
                Container(
                      ...
                      if (keyGroup.currentState?.validator() ?? true) {
                        cubit.postThongTinKhach(
                          soCMT:
                              widget.checkIdCardModel.frontContent?.content?.id,
                          coQuanToChuc:
                              coQuanToChucEditingController.text.trim(),
                          document: widget.base64Image,
                          homeTown: widget
                              .checkIdCardModel.frontContent?.content?.hometown,
                          hoVaTen: widget
                              .checkIdCardModel.frontContent?.content?.name,
                          idTheVao: idTheVaoEditingController.text,
                          residence: widget.checkIdCardModel.frontContent
                              ?.content?.residence,
                          lyDoVaoCoQuan: nhapLyDoEditingController.text.trim(),
                          nguoiTiepDon:
                              nguoiTiepDonEditingController.text.trim(),
                          context: context,
                        );
                      } else {
                        return;
