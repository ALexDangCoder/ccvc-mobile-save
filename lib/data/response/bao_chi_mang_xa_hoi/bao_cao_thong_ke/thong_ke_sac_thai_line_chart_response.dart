...
class ThongKeTheoSacThaiResponse {
  ...
  SacThaiLineChartModel toDomain() => SacThaiLineChartModel(
        trungLap: listSacThai
            .map((e) =>
                LineChartData(date: e.date ?? '', count: e.trungLap ?? 0))
            .toList(),
        tichCuc: listSacThai
            .map(
                (e) => LineChartData(date: e.date ?? '', count: e.tichCuc ?? 0))
            .toList(),
        tieuCuc: listSacThai
            .map(
                (e) => LineChartData(date: e.date ?? '', count: e.tieuCuc ?? 0))
            .toList(),
      );
}
